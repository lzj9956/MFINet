...
class DataloadVal(Dataset):
    def __init__(self, config):
        ...
        for seq_id in seq_split:
            ...
            for i in range(len(poses_list)):
                ...
                pose_diff = current_pose_inv.dot(poses_list[i])
                # self.flist.append((fname_pcd, fname_label, pose_diff, seq_id, file_id))
                self.flist.append((fname_pcd, pose_diff, seq_id, file_id))
    ...
    def form_seq(self, meta_list):
        # fname_pcd, fname_label, pose_diff, _, _ = meta_list
        fname_pcd, pose_diff, path_seq, path_name = meta_list
        # load pcd
        pcds_tmp = np.fromfile(fname_pcd, dtype=np.float32).reshape((-1, 4))
        pcds_ht = utils.Trans(pcds_tmp, pose_diff)
        # load label
        ...
        # return pcds_ht, pcds_label_use, fname_pcd, path_seq, path_name
        return pcds_ht, fname_pcd, path_seq, path_name

    def __getitem__(self, index):
        meta_list = self.flist[index]

        # load history pcds
        # pc, pc_label, fname_pcd, path_seq, path_name = self.form_seq(meta_list)
        pc, fname_pcd, path_seq, path_name = self.form_seq(meta_list)

        path_name = path_name + '.label'
        ...
        learning_map = self.task_cfg['learning_map_inv']

        # return pcds_xyzi, pcds_coord, pcds_sphere_coord, pcds_target, fname_pcd, path_seq, path_name, learning_map
        return pcds_xyzi, pcds_coord, pcds_sphere_coord, fname_pcd, path_seq, path_name, learning_map
