...
from networks.patch import extract_image_patches, reduce_mean, reduce_sum, same_padding, reverse_patches
import pdb
import math
...
class TransBlock(nn.Module):
    ...
    def forward(self, x, ksizes, padding):
        B = x.shape[0]
        x = extract_image_patches(x, ksizes, padding,
                                  strides=[1, 1],
                                  rates=[1, 1])  # 16*2304*576
        x = x.permute(0, 2, 1)
