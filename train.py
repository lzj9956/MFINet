...
def train_fp16(epoch, end_epoch, args, model, train_loader, optimizer, scheduler, logger, log_frequency):
    ...
    for i, (pcds_xyzi, pcds_coord, pcds_sphere_coord, pcds_target) in tqdm.tqdm(enumerate(train_loader)):
        ...
        with torch.cuda.amp.autocast():
            pcds_xyzi = pcds_xyzi.cuda()
            pcds_coord = pcds_coord.cuda()
            pcds_sphere_coord = pcds_sphere_coord.cuda()
            pcds_target = pcds_target.cuda()
...
if __name__ == '__main__':
    ...
    # parser.add_argument('--config', help='config file path', default='config/wce_two_stage.py', type=str)
